...
const onAccountsChanged = async (accounts) => {
  ...
  const ens = userStore.chainEnsSupport.value ? await connection.lookupAddress(address) : null
...
const persistState = async () => {
  try {
    ...
    const [address] = await connection.provider.request({ method: 'eth_accounts' })
    if (address && addressStorage.value === address && chainId === import.meta.env.VITE_NETWORK) await onAccountsChanged([address])
